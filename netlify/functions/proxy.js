...
exports.handler = async (event) => {
  ...
  // ====== PARSING BODY: MENERIMA JSON DAN MULTIPART ======
  let params = {};
  ...
  try {
    if (contentType.includes('multipart/form-data')) {
      ...
      if (!event.isBase64Encoded) buffer = Buffer.from(event.body);
      const parts = multipart.Parse(buffer, boundary);
      parts.forEach(p => {
        if (p.filename) {
          params.fileName = p.filename;
          params.fileData = p.data.toString('base64');
        } else {
          params[p.name] = p.data.toString('utf8');
        }
      });
    } else {
  ...
  try {
    switch (action) {
      ...
      case 'getData': { const { data, error } = await supabase.from('opd_data').select('*').eq('year', year); if (error) throw error; return jsonRes((data || []).map(mapDBToFront), headers); }
      case 'saveData': { const rows = JSON.parse(params.rows); if (!Array.isArray(rows)) throw new Error('Format rows tidak valid'); for (const row of rows) { const payload = mapFrontToDB({ id: row.id, opd: row.opd || '', sa: parseFloat(row.sa) || 0, evidence: row.evidence || 'Belum', qa_apip: row.qaApip || 'Belum', mri: parseFloat(row.mri) || 0, iepk: parseFloat(row.iepk) || 0, rtp: row.rtp || 'Belum', status: row.status || 'Belum', subunsurs: row.subunsurs || {}, year }); const { error } = await supabase.from('opd_data').upsert(payload, { onConflict: 'id' }); if (error) throw error; } return jsonRes({ status: 'success', message: 'Data berhasil disimpan' }, headers); }
      case 'saveField': { const { opdId, field, value } = params; if (!opdId || !field) return jsonRes({ status: 'error', message: 'Parameter opdId dan field wajib diisi' }, headers); if (!ALLOWED.includes(field)) return jsonRes({ status: 'error', message: `Field '${field}' tidak diizinkan` }, headers); const fieldMap = { 'qaApip': 'qa_apip', 'qa_apip': 'qa_apip' }; const dbField = fieldMap[field] || field; const { error } = await supabase.from('opd_data').update({ [dbField]: value }).eq('id', opdId); if (error) throw error; return jsonRes({ status: 'success', message: 'Field berhasil disimpan' }, headers); }
      case 'deleteOpd': { const { opdId } = params; if (!opdId) return jsonRes({ status: 'error', message: 'Parameter opdId wajib diisi' }, headers); if (opdId === 'all') await supabase.from('opd_data').delete().eq('year', year); else await supabase.from('opd_data').delete().eq('id', opdId); return jsonRes({ status: 'success', message: 'OPD berhasil dihapus' }, headers); }
      ...
      // ====== UPLOAD FILE: SUDAH OTOMATIS DITANGANI (JSON & MULTIPART) ======
      case 'uploadFile': {
      ...
      case 'restoreBackup': { const { fileName } = params; if (!fileName) return jsonRes({ status: 'error', message: 'Parameter fileName wajib diisi' }, headers); const folderId = await getOrCreateFolder(GOOGLE_DRIVE_ROOT_FOLDER_ID, `year_${year}`); const backupFolder = await getOrCreateFolder(folderId, 'backup'); const res = await getDrive().files.list({ q: `'${backupFolder}' in parents and name='${fileName}' and trashed=false`, fields: 'files(id)' }); if (res.data.files.length === 0) return jsonRes({ status: 'error', message: 'Backup tidak ditemukan' }, headers); const fileId = res.data.files[0].id; const content = await getDrive().files.get({ fileId, alt: 'media' }); const rows = JSON.parse(content.data); await supabase.from('opd_data').delete().eq('year', year); for (const row of rows) { const { error } = await supabase.from('opd_data').insert(row); if (error) throw error; } return jsonRes({ status: 'success', message: 'Backup berhasil dipulihkan' }, headers); }
      case 'deleteBackup': { const { fileName } = params; if (!fileName) return jsonRes({ status: 'error', message: 'Parameter fileName wajib diisi' }, headers); const folderId = await getOrCreateFolder(GOOGLE_DRIVE_ROOT_FOLDER_ID, `year_${year}`); const backupFolder = await getOrCreateFolder(folderId, 'backup'); const res = await getDrive().files.list({ q: `'${backupFolder}' in parents and name='${fileName}' and trashed=false`, fields: 'files(id)' }); if (res.data.files.length > 0) await getDrive().files.delete({ fileId: res.data.files[0].id }); return jsonRes({ status: 'success', message: 'Backup dihapus' }, headers); }
