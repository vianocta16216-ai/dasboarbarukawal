...
exports.handler = async (event) => {
  ...
  // PARSING BODY TAHAN BANTING (MENANGANI PART.BODY.PIPE)
  let params = {};
  const contentType = event.headers['content-type'] || '';
  try {
    if (contentType.includes('multipart/form-data')) {
      // Jika terlanjur multipart, kita ekstrak manual (tanpa busboy)
      let bodyBuffer;
      ...
      // Ekstrak field sederhana dari multipart
      const actionMatch = bodyText.match(/name="action"\s*\r?\n\r?\n(.*?)\r?\n/);
      if (actionMatch) params.action = actionMatch[1].trim();
      const yearMatch = bodyText.match(/name="year"\s*\r?\n\r?\n(.*?)\r?\n/);
      if (yearMatch) params.year = yearMatch[1].trim();
      const opdNameMatch = bodyText.match(/name="opdName"\s*\r?\n\r?\n(.*?)\r?\n/);
      if (opdNameMatch) params.opdName = opdNameMatch[1].trim();
      // ... (ekstrak field lain bila perlu)
    } else {
  ...
  try {
    switch (action) {
      ...
      // ====== UPLOAD FILE ======
      case 'uploadFile': {
        // Jika content-type multipart, kita perlu ekstrak fileData dari bodyText (sederhana)
        let fileData = params.fileData;
        let fileName = params.fileName;
        if (!fileData && contentType.includes('multipart/form-data')) {
          // Ekstrak file dari multipart (sederhana, tanpa library)
          let bodyBuffer;
          if (event.isBase64Encoded) {
            bodyBuffer = Buffer.from(event.body, 'base64');
          } else {
          ...
          const bodyText = bodyBuffer.toString('utf8');
          // Cari bagian file binary (ini sangat sederhana, biasanya di akhir body)
          const fileStart = bodyText.indexOf('\r\n\r\n', bodyText.indexOf('filename="'));
          if (fileStart !== -1) {
            const fileEnd = bodyText.lastIndexOf('\r\n--');
            if (fileEnd !== -1) {
              const base64Part = Buffer.from(bodyText.substring(fileStart + 4, fileEnd), 'binary').toString('base64');
              fileData = base64Part;
        ...
        const fileUrl = await uploadFileToDrive({ ...params, fileData, fileName });
