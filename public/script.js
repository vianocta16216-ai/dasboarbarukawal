...
async function openRtpEvidenceModal(id){const row=rows.find(r=>r.id===id);if(!row)return;rtpEvidenceEditingRowId=id;document.getElementById('rtpEvidenceOpdName').textContent=row.opd||'Tanpa Nama';const folderInput=document.getElementById('rtpEvidenceFolderName');if(folderInput)folderInput.value=row.rtpEvidenceFolder||'Evidence RTP';document.getElementById('rtpEvidenceModal').classList.add('active');rtpEvidenceSetStatus('Memuat daftar evidence RTP...');try{const d=await callServer('getRtpEvidence',{opdId:id,year:currentYear});row.rtpEvidence=Array.isArray(d.rtpEvidence)?d.rtpEvidence:[];row.rtpEvidenceFolder=d.folderName||row.rtpEvidenceFolder||'Evidence RTP';if(folderInput)folderInput.value=row.rtpEvidenceFolder;renderRtpEvidenceList(row);rtpEvidenceSetStatus('Folder Evidence RTP siap digunakan.','success');}catch(err){rtpEvidenceSetStatus('Gagal memuat: '+err.message,'error');}}
